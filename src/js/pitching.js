...
function ballFour() {
  ...
  if (battersQueue.length > 0) {
    ...
    const animations = advanceRunnersForBallFour(newRunner);
...
function drawSlider() {
  ...
  const gray1 = total * 0.6;
  const blue = total * 0.25;
...
function updateSlider(now = performance.now()) {
  ...
  if (slider.handleX > slider.x + slider.width) {
    ...
    animateBall(() => {
      ...
      setTimeout(() => {
        if (!preventReturnToPitcher && !runnersInMotion && !runnersInStealing) {
          returnBallToPitcher();
        }
        preventReturnToPitcher = false;
      }, 800);
    ...
    setTimeout(() => {
        ...
        const waitForBallDone = setInterval(() => {
            if (!ball.active) {
                ...
                if (typeof aiBattingEnabled !== "undefined" && aiBattingEnabled) return;
                evaluateResult();
            }
        }, 50);
    }, delay);
